...
class AgentState(TypedDict):
    ...
    order_id: int | None        # ناتج extractor — «ملتصق» (Sticky)، انظر أدناه
...
#### سلوك الحالة بين الدورات (معتمد)

- **`order_id` «ملتصق» (Sticky):** الدورات اللاحقة في نفس الجلسة تسترجع الحالة من
  الذاكرة الدائمة بدل تمرير قيم ابتدائية كاملة (انظر 8.3). إذا لم تحتوِ رسالة العميل
  الجديدة على رقم طلب، **لا يُطمس** `order_id` السابق بـ `None` — يُحتفظ به حتى يستطيع
  العميل أن يسأل «ومتى يوصل؟» بلا إعادة ذكر الرقم. يُستبدل الرقم فقط عند ورود رقم جديد
  في الرسالة. التنفيذ في `graph.extract_info` (`run_turn` يمرّر `turn_input` بحقلين فقط
  في الدورات اللاحقة).
- **`product_name` غير ملتصق:** يُحدَّث بما تُخرجه الرسالة الحالية فقط. عدم التناظر مع
  `order_id` **مقصود** لأن الأدوات لا تستهلك `product_name` بنفس طريقة استهلاك
  `check_order_status` لـ `order_id`.
- **`tool_result` و `needs_escalation` «لكل دورة»:** تُصفَّران في بداية كل دورة داخل
  `classify_message` (أول عقدة في كل مسار) لمنع تسرّب نتيجة استعلام قديم إلى ردّ لا
  علاقة له به، ولمنع بقاء `needs_escalation = True` من شكوى سابقة في دورة ليست شكوى.
...
- **`extract_info` لا يطمس `order_id`:** إذا لم تُخرج الرسالة الحالية رقماً، يُحتفظ بالقيمة
  المسترجعة من الذاكرة (سلوك Sticky — تفصيله في 8.1).
...
> `tool_result`، مع ثلاثة أمثلة قصيرة (few-shot) تشمل مثال رفض صريح عند غياب المعلومة.
> سبب إضافة الحظر: في اختبار Phase 5 صُنِّفت رسالة سؤال عن مدة الإرجاع `other` فلم
> تُستدعَ `fetch_policy`، ومع ذلك ادّعى الرد أن المدة «14 يوم» (قيمة صحيحة مصادفةً) —
> أي اختلاق يخالف قاعدة عدم الاختراع. **التنفيذ يستخدم قيماً فعلية في المثالين
