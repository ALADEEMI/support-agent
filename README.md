# نظام وكيل محادثة لخدمة العملاء — متجر النخبة

وكيل محادثة عربي لخدمة العملاء (لهجة يمنية مهيمنة) لمتجر إلكتروني وهمي، وفق الوثيقة المعتمدة `docs/ADR.md`.

## المكوّنات

- `backend/` — Flask + LangGraph + SQLite + نموذج RNN (BiLSTM)
- `frontend/` — واجهة React
- `training/` — نوتبوك تدريب الموديل وتقارير التقييم (يُشغَّل يدوياً على Google Colab)
- `docs/` — وثيقة الـ ADR

## المتطلبات المسبقة

- Python **3.12** — TensorFlow لا يوفر حزماً لإصدار 3.14 حالياً، لذلك تُنشأ البيئة الافتراضية بـ `py -3.12`
- Node.js 18+ و npm
- git

## التشغيل بضغطة واحدة (Windows)

يوجد سكربت تشغيل جاهز في جذر المشروع: **`run_project.bat`** — انقر عليه نقراً مزدوجاً
لتشغيل النظام كاملاً (Backend + Frontend) دون أي أوامر يدوية.

```powershell
.\run_project.bat
```

**ما الذي يفعله السكربت؟**

1. **يتحقق من المتطلبات:** وجود `python` و`node` و`npm` في PATH.
2. **يتحقق من `.env`:** إن لم يكن موجوداً في الجذر ينسخه تلقائياً (من `backend\.env` إن وُجد،
   وإلا من `.env.example`) — **ولا يستبدل ملف `.env` موجوداً أبداً**. وإن كان
   `COMMANDCODE_API_KEY` فارغاً يحذّرك ويسألك قبل المتابعة.
3. **بيئة بايثون:** يستخدم `.venv` أو `venv` إن وُجدت، وإلا ينشئ `.venv` تلقائياً
   (يفضّل `py -3.12` لعدم توفر TensorFlow لإصدار 3.14)، ثم يتحقق من الاعتماديات
   ويثبّتها من `backend/requirements.txt` عند نقصها.
4. **اعتماديات الواجهة:** ينفّذ `npm install` داخل `frontend/` إن كانت `node_modules` مفقودة.
5. **قاعدة البيانات:** ينفّذ `backend/database/seed_data.py` تلقائياً إن كانت `support.db` مفقودة.
6. **يشغّل الخدمتين** في نافذتين منفصلتين بعنوانين واضحين:
   - `Support Agent - Backend (Flask/LangGraph)` على المنفذ **5000**
   - `Support Agent - Frontend (React)` على المنفذ **3000**
7. **يفتح المتصفح** تلقائياً على `http://localhost:3000`، ثم يطبع ملخصاً بالعناوين والمنافذ
   وكيفية الإيقاف.

**الإيقاف:** أغلق نافذتي الخدمتين (أو `Ctrl+C` داخل كل منهما) — إغلاقهما يحرّر المنفذين 5000 و3000.

**وضع الفحص (Dry run):** لرؤية كل الفحوصات وما سينفّذه السكربت **بدون** تشغيل أي خدمة:

```powershell
.\run_project.bat /dryrun
```

**ملاحظات:**
- كل المسارات تُحسب نسبةً إلى موقع السكربت نفسه، فيعمل من أي مكان.
- لا يُنشئ السكربت أي ملفات إضافية ولا يلمس أي ملف حساس.
- إن كان المنفذ 5000 أو 3000 مشغولاً فسيطبع تحذيراً واضحاً.

## الإعداد

### 1) متغيرات البيئة

```powershell
Copy-Item .env.example .env
```

ثم عبّئ القيم داخل `.env` (خصوصاً `COMMANDCODE_API_KEY`). لا يوضع أي مفتاح داخل الكود ولا يُرفع لأي نظام تحكم إصدار.

### 2) بيئة بايثون (Python 3.12)

```powershell
py -3.12 -m venv .venv
.\.venv\Scripts\Activate.ps1
python -m pip install -r backend/requirements.txt
```

### 3) قاعدة البيانات (SQLite)

تُنشأ القاعدة وتُعبّأ ببيانات وهمية حتمية (منتجات، عملاء، طلبات، سياسات) بأمر واحد:

```powershell
cd backend
python database/seed_data.py
```

- المسار الافتراضي: `backend/database/support.db` (يُتحكم به عبر `DATABASE_PATH` في `.env`).
- الأمر آمن للتكرار: يمسح بيانات الجداول ثم يعيد إدخالها بالقيم نفسها في كل مرة (لا يتضاعف شيء).
- الملف الناتج ملف واحد قابل للنسخ، ولا يُرفع إلى git لأنه بيانات مُولَّدة (يمكن إعادة بنائه بالأمر أعلاه).

**محتوى البيانات التجريبية:** 6 عملاء، 15 منتجاً (القائمة المعتمدة)، 15 طلباً بأرقام ثابتة من `1001` إلى `1015`، و3 سياسات (`return`، `refund`، `shipping`).

أرقام الطلبات الصالحة للاختبار موزعة على كل الحالات:

| حالة الطلب | أمثلة أرقام |
|---|---|
| قيد التجهيز | 1001، 1007، 1012 |
| قيد الشحن | 1002، 1005، 1009، 1014 |
| تم التوصيل | 1003، 1006، 1008، 1010، 1013، 1015 |
| ملغي | 1004، 1011 |

أرقام الطلبات غير الموجودة في هذا النطاق تُستخدم لاختبار حالة «الطلب غير موجود».

### 4) الواجهة الخلفية (Flask)

```powershell
cd backend
python app.py
```

فحص سريع: `http://localhost:5000/health` يجب أن يعيد `{"status": "ok"}`.

#### توثيق الـ API

**`POST /api/chat`** — يستقبل رسالة عميل ويعيد رد الوكيل، ويحفظ الطرفين في `chat_messages`.

جسم الطلب (JSON):

| الحقل | النوع | مطلوب | الوصف |
|---|---|---|---|
| `session_id` | string | نعم | معرّف الجلسة (يُستخدم أيضاً كـ `thread_id` لذاكرة الـ Graph) |
| `message` | string | نعم | رسالة العميل |

```json
{ "session_id": "user-123", "message": "وين وصل طلبي رقم 1002؟" }
```

استجابة النجاح (200):

| الحقل | النوع | الوصف |
|---|---|---|
| `session_id` | string | نفس المعرّف المُرسل |
| `reply` | string | رد الوكيل النصي |
| `category` | string | الفئة المصنّفة (`complaint` / `order_inquiry` / `return_request` / `other`) |
| `order_id` | int \| null | رقم الطلب المستخرج |
| `product_name` | string \| null | المنتج المستخرج |
| `needs_escalation` | bool | هل أُنشئت تذكرة |

```json
{
  "session_id": "user-123",
  "reply": "حياك الله يا غالي، طلبك رقم 1002 حالياً قيد الشحن...",
  "category": "order_inquiry",
  "order_id": 1002,
  "product_name": "شاحن سريع",
  "needs_escalation": false
}
```

أخطاء (كلها بصيغة `{"error": "..."}`):

| الرمز | السبب |
|---|---|
| 400 | جسم ليس JSON، أو `session_id`/`message` مفقود أو فارغ |
| 500 | `COMMANDCODE_API_KEY` غير موجود بالبيئة، أو خطأ داخلي |
| 502 | فشل الوصول إلى مزوّد الـ LLM |

مثال (PowerShell، ثلاث رسائل متتالية بنفس الجلسة):

```powershell
$body = @{ session_id = "user-123"; message = "وين وصل طلبي رقم 1002؟" } | ConvertTo-Json
Invoke-RestMethod -Uri http://localhost:5000/api/chat -Method Post -ContentType "application/json" -Body $body
```

**`GET /api/history/<session_id>`** — يرجع سجل المحادثة الدائم لجلسة (لعرضه عند فتح الصفحة).

```json
{
  "session_id": "user-123",
  "messages": [
    { "message_id": 1, "sender": "customer", "content": "وين وصل طلبي رقم 1002؟", "timestamp": "2026-09-11 17:16:52" },
    { "message_id": 2, "sender": "agent", "content": "حياك الله يا غالي...", "timestamp": "2026-09-11 17:16:58" }
  ]
}
```

- `sender` إما `customer` أو `agent`، والرسائل مرتّبة زمنياً.
- جلسة بلا سجل تُرجع `messages: []` برمز 200 (ليست خطأ).
- رمز 500 عند فشل قراءة قاعدة البيانات.

### 5) الواجهة الأمامية (React)

```powershell
cd frontend
npm install
npm start
```

تُفتح على `http://localhost:3000`. الواجهة تحتفظ بمعرّف الجلسة في `localStorage` وتحمّل السجل
القديم تلقائياً عند فتح الصفحة. متغيّر `REACT_APP_API_URL` (انظر `frontend/.env.example`) يحدد
عنوان الـ backend، وقيمته الافتراضية `http://localhost:5000` إن لم يُضبط.

> تنبيه: شغّل الـ backend قبل الواجهة، فبدونه ستفشل الرسائل (وتظهر رسالة خطأ في الواجهة).

## حالة المشروع

**جميع مراحل ADR الثماني (0–7) مكتملة ومنفَّذة ومتحقَّق منها فعلياً.**

| المرحلة | الحالة |
|---|---|
| 0 — Bootstrap | ✅ |
| 1 — توليد وتنظيف الداتا سيت | ✅ |
| 2 — تدريب وتقييم نموذج RNN | ✅ |
| 3 — قاعدة البيانات | ✅ |
| 4 — طبقة الوكيل (Graph + Tools + Extractor) | ✅ |
| 5 — Flask Backend | ✅ |
| 6 — React Frontend | ✅ |
| 7 — اختبار شامل وتوثيق نهائي | ✅ |

**نتائج التحقق النهائية:** اختبارات الوحدة **71/71 ناجحة** · سيناريوهات E2E **10/10 ناجحة** ·
نموذج `BiLSTM_Baseline` بدقة **0.9450** و Macro F1 **0.9449** على مجموعة الاختبار.

## التوثيق

| الملف | المحتوى |
|---|---|
| `docs/ADR.md` | الوثيقة المعمارية المعتمدة (محدَّثة لتطابق التنفيذ الفعلي) |
| `docs/PROJECT_REPORT.md` | **تقرير المشروع النهائي** (ملخص، نتائج المراحل، القيود، الفهرس) |
| `PROGRESS.md` | سجل التقدّم والقرارات الهندسية والمخاطر |
| `docs/phase7_e2e_report.md` | نتائج 10 سيناريوهات end-to-end بالردود الفعلية |
| `docs/phase6_ui_screenshot.png` | لقطة شاشة الواجهة النهائية |
| `training/reports/` | تقارير الداتا سيت ومقارنة النماذج ومصفوفات الالتباس |

## فحص سريع مقترح للعرض

```powershell
cd backend
python database/seed_data.py          # تجهيز القاعدة (6 عملاء / 15 منتجاً / 15 طلباً / 3 سياسات)
python app.py                         # الـ backend على :5000
# في طرفية أخرى:
cd frontend
npm start                             # الواجهة على :3000
```

جرّب في الواجهة: «وين وصل طلبي رقم 1002؟» ثم «طيب متى يوصل؟» (لإظهار الذاكرة)،
ثم «ابغى ارجع المنتج لان مو مطابق للمواصفات» (سياسات)، ثم شكوى (إنشاء تذكرة).

## المراحل

حالة تنفيذ المراحل موثّقة في `PROGRESS.md`، وتفصيلها في `docs/ADR.md` قسم 10.
